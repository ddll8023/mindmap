...
- **全屏模式** — 将组件扩展为全屏显示
- **LaTeX 数学公式** — 渲染 `$...$` 行内公式，以及同行或标准跨行的 `$$...$$` 块级公式，使用纯 SVG 路径，支持高清 PNG 导出
...
- **多根节点** — 在同一画布上构建多棵独立的树
- **拖拽操作** — 拖拽重新排列兄弟节点、跨越中线重新平衡两侧，或将节点拖到其他节点上重新挂载子树
- **撤销 / 重做** — 为可视化编辑、导入、拖拽排序/改层级、剪切/粘贴和程序化替换提供画布级历史
- **标签筛选** — 按标签筛选节点，并在保留祖先上下文的同时淡化无关分支
...
- **工具栏控制** — 通过 `toolbar` prop 显示/隐藏缩放、历史和标签控件
...
工具栏包括缩放/历史控件（左下角）、标签筛选（左上角），以及文本模式/全屏切换按钮（右下角）。`toolbar` prop 支持 `zoom`、`history` 和 `tags` 标志；文本模式和全屏按钮始终可用。
...
- **单指在画布上** — 平移视图
- **单指在节点上** — 拖拽重新排列兄弟节点，或拖到其他节点上重新挂载子树
